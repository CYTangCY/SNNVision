...
int lif_network::linenum_neuronParameter(const char *par)
{
    int linenum = 0;
    float f[6];
    FILE *fp = fopen("iq-neuron/inputs/neuronParameter_LIF.txt", "r");
    if(fp == NULL) {
...
{
    int i;
    float g, rest, threshold, reset, noise;
    FILE *fp;
    /*
    if(_tau == 0) {
    ...
    */
    fp = fopen("iq-neuron/inputs/neuronParameter_LIF.txt", "r");
    while(fscanf(fp, " %d %f %f %f %f %f", &i, &g, &rest, &threshold, &reset, &noise) == 6) {
...
int lif_network::get_weight(const char *con)
{
    ...
    fp = fopen("iq-neuron/inputs/Connection_Table_LIF.txt", "r");
    if(fp == NULL) {
...
void lif_network::printfile(FILE **fp_potential, FILE **fp_inputcurrent,FILE **fp_spikerate)
{
    int i;
    ...
    for(i = 0; i < _num_neurons; i++) {
        fprintf(fp_inputcurrent[i], "%f\n", (_neurons+i)->inputPnoise());
    }
    for(i = 0; i < _num_neurons; i++) {
        fprintf(fp_spikerate[i], "%f\n", (_neurons+i)->spike_rate());
    } 
    return;
}
...
int lif_network::set_neuron(int neuron_index, float g, float rest,
                            float threshold, float reset, float noise)
{
    if(neuron_index >= 0 && neuron_index < _num_neurons) {
...
float lif_network::inputPnoise(int neuron_index)
{
	return (_neurons + neuron_index)->inputPnoise();
}
...
extern "C"
{
    ...
    DLLEXPORTLIF int lif_network_set_neuron(lif_network* network, int neuron_index, float g, float rest, float threshold, float reset, float noise) {return network->set_neuron(neuron_index, g, rest, threshold, reset, noise);}
    ...
    DLLEXPORTLIF float lif_network_inputPnoise(lif_network* network, int neuron_index) {return network->inputPnoise(neuron_index);}
