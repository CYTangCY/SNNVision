...
@jit
def xyz(wall, K, Rt_translate):
	hg = 40//2
	arrayidealVectorT = np.zeros((640,480,2))
	for point in wall:	 
...
@jit
def RYP(wall, K, Rt_rotate):
	hg = 40//2
	arrayidealVectorRT = np.zeros((640,480,2))
	for point in wall:	
		x, y = int(point[0]), int(point[1]) 
		#print('x:',x)
...
def reshapearray(flow):
	flow = flow.reshape((640, 480, 2))
	return flow
...
def meanFrameDifference(diff):
	diff = diff.reshape((640, 480))
	sh = (8, 60, 8, 80)
